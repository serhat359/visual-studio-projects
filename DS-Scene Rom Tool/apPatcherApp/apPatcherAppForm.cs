    ...
    public class apPatcherAppForm : Form
    {
        ...
        private void action_exportRom()
        {
            if (!this.checkBoxApPatch.Checked || (this.patchInfo != null))
            {
                ...
                while (romOut == this.txtFileLoc.Text)
                {
        ...
        public void addGameToCMPList(cmpGameListItem item)
        {
            ...
            {
                if (((item.region != "") && (item.name != "")) && ((item.hash != "") && (item.gameCode != "")))
                {
        ...
        private void btnExportAP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    outPutFolder = this.txtFileLoc.Text.Substring(0, this.txtFileLoc.Text.LastIndexOf('/') + 1);
                }
                else
                {
                    outPutFolder = this.txtFileLoc.Text.Substring(0, this.txtFileLoc.Text.LastIndexOf('\\') + 1);
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        using (StreamWriter writer = new StreamWriter(path))
                        {
                            ...
                            int index = 0;
                            for (index = 0; index < this.patchInfo.patchlines; index++)
                            {
                            ...
                            if (newExt == "dsapf")
                            {
                                string str5 = this.run.hexAndMathFunction.convertHexToEncryptionKey("790077003F0028003F0050003F003F00");
                                GCHandle gch = GCHandle.Alloc(str5, GCHandleType.Pinned);
                                this.encryptor.EncryptFile(path, fn, str5, gch);
        ...
        {
            if (System.IO.File.Exists("data/header_cache/" + this.romHeader.romHeader.hash.Substring(0, 1) + "/" + this.romHeader.romHeader.hash + ".dsrhd"))
            {
        ...
        private void btnImportAP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (this.fdlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        string str = this.getFileExtension(this.fdlg.FileName);
                        FileStream fs = new FileStream(this.fdlg.FileName, FileMode.Open, FileAccess.Read);
                        if (str == "dsapf")
                        {
                        ...
                        {
                            string str6;
                            while ((str6 = reader2.ReadLine()) != null)
                            {
                                try
                                {
                                    string[] strArray2 = str6.Split(new char[] { '[' });
                                    string str7 = strArray2[0];
                                    strArray2 = strArray2[1].Split(new char[] { ']' });
                                    if (strArray2[0].Length == 8)
                                    {
                                        class2 = this.patchDb.readPatchFromStream(reader2, str7, strArray2[0], strArray2[1]);
                                    }
                                    continue;
                                }
                                catch
        ...
        private void buttonSaveIcon_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < (this.txtFileLoc.Text.Length - 1); i++)
            {
                if (this.txtFileLoc.Text.Substring((this.txtFileLoc.Text.Length - i) - 1, 1) == @"\")
                {
                    dialog.FileName = this.txtFileLoc.Text.Substring(this.txtFileLoc.Text.Length - i, this.txtFileLoc.Text.Length - (this.txtFileLoc.Text.Length - i));
                    if ((dialog.FileName.Substring(dialog.FileName.Length - 4, 4) == ".nds") || (dialog.FileName.Substring(dialog.FileName.Length - 4, 4) == ".3ds"))
                    {
                        ...
                        {
                            if (this.getPalCheckBox(j).Checked)
                            {
        ...
        private string changeFileExt(string fn, string newExt)
        {
            int num = 0;
            num = 0;
            while (num < (fn.Length - 1))
            {
                if (fn.Substring((fn.Length - num) - 1, 1) == ".")
                {
                    break;
                }
                num++;
        ...
        private void checkAppUpdate(bool download)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show(exception.Message, "checkAppUpdate failed to parse new info", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void checkCMPDatabaseUpdate(bool download, cmpDownloadType type = 0)
        {
            ...
            if (this.downloadFile(url, "data/temp/", "Update Check", "", null, null))
            {
                if (this.checkCMPVersionTxt(download, type) && download)
                {
                    this.action_loadDatabase();
                }
            }
        ...
        private bool checkCMPVersionTxt(bool download, cmpDownloadType type)
        {
            ...
            if (!System.IO.File.Exists("data/databases/version_cmp.bin"))
            {
                for (int k = 0; k < 20; k++)
                {
                    infoArray2[k] = new updateCSVInfo();
                    infoArray2[k].ver = "0";
                }
                num2 = index;
            }
            else
            {
                try
                {
                    string str4 = this.run.hexAndMathFunction.convertHexToEncryptionKey("3F006F003F003F001000000069003F00");
                    FileStream stream3 = new FileStream("data/databases/version_cmp.bin", FileMode.Open, FileAccess.Read);
                    using (StreamReader reader2 = new StreamReader(this.encryptor.createDecryptionReadStream(str4, stream3)))
                    {
                        string str5;
                ...
                catch (Exception exception2)
                {
                    ...
                    {
                        MessageBox.Show(string.Concat(new object[] { "checkCMPVersionTxt failed to load cur info [", num2, "/", 20, "]:\r\n ", exception2.Message }));
                        this.toolStripStatusLabel.Text = "Update Failed";
                        return false;
                    }
                }
            ...
            for (int i = 0; i < 20; i++)
            {
                this.cmpCheckFile[i].update_available = false;
                if (i >= index)
                {
                    break;
                }
                ...
                {
                    this.cmpCheckFile[i].update_available = true;
                }
            }
            for (int j = 0; j < 20; j++)
            {
                ...
                if (infoArray2[j].ver != infoArray[j].ver)
                {
                    if (!flag2 && (download || ((j + 1) == (int)type)))
                    {
                        this.updateViewer.formSetup("cmp", infoArray[j].ver);
                        switch (this.updateViewer.ShowDialog(this))
                        {
                            case DialogResult.Yes:
                    ...
                    if ((j + 1) == (int)none)
                    {
                        url = "http://files-ds-scene.net/romtool/download.php?f=releases/cmp/" + infoArray[j].fn;
                        if (!this.downloadFile(url, "data/databases/", infoArray[j].fn, "", null, null))
                        {
                            this.toolStripStatusLabel.Text = "CMP Update Failed";
                            return false;
                        }
                        string str10 = text;
                        text = str10 + infoArray[j].fn + " [Updated to " + infoArray[j].ver + "]\r\n";
                        infoArray2[j].fn = infoArray[j].fn;
                        infoArray2[j].ver = infoArray[j].ver;
                        flag = true;
                    }
                    else
                    {
                        string str11 = text;
                        text = str11 + infoArray[j].fn + " [Available for update " + infoArray[j].ver + "]\r\n";
                    }
                }
                else
                {
                    string str12 = text;
                    text = str12 + infoArray[j].fn + " [" + infoArray[j].ver + " Already Installed]\r\n";
                }
            }
            if (flag)
            {
                try
                {
                    ...
                    for (int m = 0; m < 20; m++)
                    {
                        ...
                        if ((infoArray2[m].ver != "0") && (infoArray2[m].ver != ""))
                        {
                            if ((ver != "") && (ver == infoArray2[m].ver))
                            {
                                num6++;
                            }
                            else
                            ...
                            ver = infoArray2[m].ver;
                        }
                        string str13 = str7;
                        str7 = str13 + infoArray2[m].fn + "|" + infoArray2[m].ver + "\r\n";
                    }
                    if (num6 == index)
                    {
                        flag3 = false;
                ...
                try
                {
                    if (System.IO.File.Exists("data/temp/version_cmp.txt"))
                    {
                        ...
                        string str9 = this.run.hexAndMathFunction.convertHexToEncryptionKey("3F006F003F003F001000000069003F00");
                        GCHandle gch = GCHandle.Alloc(str9, GCHandleType.Pinned);
                        this.encryptor.EncryptFile("data/temp/version_cmp.txt", "data/databases/version_cmp.bin", str9, gch);
        ...
        private bool checkVersionTxt(bool download)
        {
            ...
            if ((!System.IO.File.Exists("data/databases/version.bin") || !System.IO.File.Exists("data/databases/offsets.dsapdb")) || !this.databasesOk)
            {
                for (int j = 0; j < 20; j++)
                {
                    infoArray2[j] = new updateCSVInfo();
                    infoArray2[j].ver = "0";
                }
                num2 = index;
            }
            else
            {
                try
                {
                    ...
                    FileStream stream3 = new FileStream("data/databases/version.bin", FileMode.Open, FileAccess.Read);
                    using (StreamReader reader2 = new StreamReader(this.encryptor.createDecryptionReadStream(str4, stream3)))
                    {
                        string str5;
                        while ((str5 = reader2.ReadLine()) != null)
                ...
                catch (Exception exception2)
                {
                    if (exception2.Message.Substring(0, 14) != "Could not find")
                    {
                        MessageBox.Show(string.Concat(new object[] { "checkVersionTxt failed to load cur info [", num2, "/", 20, "]:\r\n ", exception2.Message }));
                        this.toolStripStatusLabel.Text = "AP Update Failed";
                        return false;
                    }
                }
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                if (infoArray2[i].ver != infoArray[i].ver)
                {
                    ...
                    {
                        string str7 = text;
                        text = str7 + infoArray[i].fn + " [Updated to " + infoArray[i].ver + "]\r\n";
                        flag = true;
                        continue;
                    }
                    this.toolStripStatusLabel.Text = "AP Update Failed";
                ...
                string str8 = text;
                text = str8 + infoArray[i].fn + " [" + infoArray[i].ver + " Already Installed]\r\n";
            }
            if (flag)
            {
                try
        ...
        private void collectionBrowserToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void displayRomHeaderInfo()
        {
            ...
            {
                this.enableTabs();
                string path = "data/header_cache/" + this.romHeader.romHeader.hash.ToUpper().Substring(0, 1) + "/" + this.romHeader.romHeader.hash.ToUpper() + ".dsrhd";
        ...
        private void displayRomHeaderInfo_3DS()
        {
            ...
            this.fileSizeText.Text = string.Concat(new object[] { this.run.hexAndMathFunction.bytesToGbit(this.romHeader.romHeader.fileSize_3ds), "Gb (", this.romHeader.romHeader.fileSize_3ds, " bytes)" });
            this.cardSizeText.Text = string.Concat(new object[] { this.run.hexAndMathFunction.bytesToGbit(this.romHeader.romHeader.cardSize_3ds), "Gb (", this.romHeader.romHeader.cardSize_3ds, " bytes)" });
                    ...
                    foreach (webInfo.webInfoItemClass class3 in class2.item)
                    {
                        ...
                        switch (class3.key)
                        {
                            ...
                            case "wifi":
                                ...
                                this.webInfo_wifi.Visible = true;
                                goto Label_0347;
        ...
        public bool downloadFile(string url, string dirdest, string progressTxt, string saveas = "", ProgressBar progress = null, Label status = null)
        {
            ...
            try
            {
                ...
                {
                    if ((url.Substring(url.Length - i, 1) == "/") || (url.Substring(url.Length - i, 1) == @"\"))
                    {
                        this.downloadedDataName = url.Substring((url.Length - i) + 1, (url.Length - (url.Length - i)) - 1);
                        break;
                    }
                }
            ...
            Label_0139:
                ...
                if (num3 == 0)
                {
                    ...
                    status.Text = string.Concat(new object[] { "Completed: ", progressTxt, " ", this.run.hexAndMathFunction.getPercentage(progress.Value, contentLength), "%" });
                    Application.DoEvents();
                }
                else
                {
                    stream2.Write(buffer, 0, num3);
                    if ((progress.Value + num3) <= progress.Maximum)
                    {
                        if ((progress.Value + num3) > progress.Maximum)
                        {
                            progress.Value = progress.Maximum;
                        ...
                        status.Text = string.Concat(new object[] { "Downloading: ", progressTxt, " ", this.run.hexAndMathFunction.getPercentage(progress.Value, contentLength), "%" });
                        Application.DoEvents();
                    }
                    goto Label_0139;
                }
        ...
        public bool downloadInfoFromWebsite(crcDupes.romTypes type, string hash, ProgressBar progress, Label status, bool display, bool fromTheButton = false)
        {
            ...
            {
                foreach (webInfo.webInfoItemClass class2 in this.web.info.item)
                {
                    if ((class2 != null) && (class2.key == "date"))
                    {
                        TimeSpan span;
                        DateTime utcNow = DateTime.UtcNow;
                        int year = int.Parse(class2.val.Substring(0, 4));
                        int month = int.Parse(class2.val.Substring(4, 2));
                        int day = int.Parse(class2.val.Substring(6, 2));
                        int hour = int.Parse(class2.val.Substring(8, 2));
                        int minute = int.Parse(class2.val.Substring(10, 2));
                        int second = int.Parse(class2.val.Substring(12, 2));
            ...
            {
                if ((this.options.getValue("auto_info_dl") == "1") || fromTheButton)
                {
                    if (this.web.downloadRomInfo(hash, progress, status))
                    {
                        flag = true;
            ...
            if (((class3 == null) || (class3.item[0] == null)) || ((class3.item[0].key == "error:hash not found") || (class3.item[0].key == "error:bad hash")))
            {
                this.romHeader.romHeader.website_knows_it = false;
                return flag;
            }
        ...
        private void drawIconPalette()
        {
            for (int i = 0; i < 0x10; i++)
            {
                ...
                int blue = 0;
                red = (num2 % 0x20) * 8;
                green = ((num2 / 0x20) % 0x20) * 8;
                blue = ((num2 / 0x400) % 0x20) * 8;
        ...
        private string fixFileExistsName(string fn, int count)
        {
            ...
            object obj2 = fn.Substring(0, startIndex);
            return string.Concat(new object[] { obj2, "(", count, ")", str });
        }

        private string fixFileNameNoExt(string fn, string ext_options)
        {
            string str = "";
            for (int i = ext_options.Length - 1; i >= 0; i--)
            {
                string str2 = ext_options.Substring(i, 1);
                if (str2 == "*")
                {
                    break;
                }
        ...
        public string getFileExtension(string fn)
        {
            ...
            {
                if (fn.Substring((fn.Length - 1) - i, 1) == ".")
                {
                    return fn.Substring(fn.Length - i, i);
                }
            }
            return "";
        }
        ...
        {
            if (trim == romTrimTypes.safe)
            {
                return this.getSafeTrimPosition(romIn, progress, status, true);
            }
            if (trim != romTrimTypes.none)
            {
                return this.getAccurateTrimPosition(romIn, progress, status, true);
            }
            return this.getAccurateTrimPosition(romIn, progress, status, false);
        }
        ...
        private long getSafeTrimPosition(string romIn, ProgressBar progress, Label status, bool trim)
        {
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(input))
                {
                    ...
                    Application.DoEvents();
                    int count = 0x400;
                    for (long i = reader.BaseStream.Length; i > -count; i -= count)
                    {
                        if (i < count)
                        {
                            i = count;
                        }
                        reader.BaseStream.Position = i - count;
        ...
        private void InitializeComponent()
        {
            ...
            this.menuStrip1.Items.AddRange(new ToolStripItem[] { this.fileToolStripMenuItem, this.databaseToolStripMenuItem, this.optionsToolStripMenuItem, this.aboutToolStripMenuItem });
            ...
            this.fileToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.openToolStripMenuItem, this.batchProcessToolStripMenuItem, this.nFOViewerToolStripMenuItem, this.exitToolStripMenuItem });
            ...
            this.batchProcessToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.patchingToolStripMenuItem });
            ...
            this.databaseToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.aPDatabaseToolStripMenuItem, this.cMPDatabaseToolStripMenuItem1, this.romCollectionsToolStripMenuItem });
            this.databaseToolStripMenuItem.Name = "databaseToolStripMenuItem";
            this.databaseToolStripMenuItem.Size = new Size(0x4f, 20);
            this.databaseToolStripMenuItem.Text = "Databases";
            this.aPDatabaseToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.checkForUpdatesToolStripMenuItem1, this.updateAlertsToolStripMenuItem, this.versionInfoToolStripMenuItem1 });
            ...
            this.updateAlertsToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.enableUpdateAlertsToolStripMenuItem });
            ...
            this.cMPDatabaseToolStripMenuItem1.DropDownItems.AddRange(new ToolStripItem[] { this.installToMediaToolStripMenuItem, this.toolStripSeparator2, this.checkForUpdatesToolStripMenuItem, this.updateAlertsToolStripMenuItem1, this.versionInfoToolStripMenuItem });
            ...
            this.installToMediaToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.cycloDSCMPToolStripMenuItem, this.edgeCMPToolStripMenuItem, this.uSRCHEATToolStripMenuItem });
            ...
            this.updateAlertsToolStripMenuItem1.DropDownItems.AddRange(new ToolStripItem[] { this.cycloDSToolStripMenuItem, this.eDGEToolStripMenuItem, this.uSRcheatToolStripMenuItem1 });
            ...
            this.romCollectionsToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.collectionBrowserToolStripMenuItem, this.optionsToolStripMenuItem1 });
            ...
            this.optionsToolStripMenuItem1.DropDownItems.AddRange(new ToolStripItem[] { this.autoOpenBrowserToolStripMenuItem, this.autoLaunchEmulatorToolStripMenuItem });
            ...
            this.optionsToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.emulatorLocationsToolStripMenuItem, this.applicationUpdatesToolStripMenuItem, this.dSSceneContentToolStripMenuItem });
            ...
            this.applicationUpdatesToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.checkForUpdateToolStripMenuItem1, this.toolStripSeparator4, this.disableCheckOnStartToolStripMenuItem });
            ...
            this.dSSceneContentToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.disableWebContentToolStripMenuItem, this.toolStripSeparator1, this.autoDownloadToolStripMenuItem, this.disableWebAlertContentToolStripMenuItem, this.rememberDSSceneInfoToolStripMenuItem });
            ...
            this.aboutToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[] { this.homeToolStripMenuItem, this.changeLogToolStripMenuItem });
        ...
        private void loadCMPVersionTxt()
        {
            ...
            for (num = 0; num < 4; num++)
            {
                this.cmpCheckFile[num] = new cmpCheckFileType();
                this.cmpCheckFile[num].installed = false;
                this.cmpCheckFile[num].update_available = false;
            }
            try
            {
                string sKey = this.run.hexAndMathFunction.convertHexToEncryptionKey("3F006F003F003F001000000069003F00");
                FileStream fs = new FileStream("data/databases/version_cmp.bin", FileMode.Open, FileAccess.Read);
                CryptoStream stream = this.encryptor.createDecryptionReadStream(sKey, fs);
                updateCSVInfo[] infoArray = new updateCSVInfo[20];
                num = 0;
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    while ((str2 = reader.ReadLine()) != null)
                    {
                        ...
                        infoArray[num] = new updateCSVInfo();
                        infoArray[num].fn = strArray[0];
                        infoArray[num].ver = strArray[1];
                        if ((infoArray[num].ver != "0") && (infoArray[num].ver != ""))
                        {
                            this.cmpCheckFile[num].installed = true;
                            this.cmpCheckFile[num].fn = infoArray[num].fn;
                            this.cmpCheckFile[num].ver = infoArray[num].ver;
                        }
        ...
        public string longFileName(string fn, int dirlen)
        {
            ...
            length++;
            return (fn.Substring(0, (0xf8 - length) - dirlen) + fn.Substring(fn.Length - length, length));
        }
        ...
        private string openSaveDialogue()
        {
            ...
            if (this.romHeader.romHeader.is3DS)
            {
                dialog.Filter = "(Nintendo 3DS Rom *." + newExt + ")|*." + newExt;
            }
            else
            {
                dialog.Filter = "(Nintendo DS Rom *." + newExt + ")|*." + newExt;
            }
            string outPutFolder = "";
            if (this.txtFileLoc.Text.Replace('/', '+') != this.txtFileLoc.Text)
            {
            ...
            if (this.romHeader.romHeader.is3DS)
            {
                return this.changeFileExt(fn, "3ds");
            }
            return this.changeFileExt(fn, "nds");
        }
        ...
        public string origFileLocToNewFileName(string origfn, bool trim, bool apPatch, string outPutFolder)
        {
            int num2;
            string str = "";
            for (int i = 0; i < (origfn.Length - 1); i++)
            {
                switch (origfn.Substring((origfn.Length - i) - 1, 1))
                {
                    case @"\":
                    case "/":
                        str = origfn.Substring(origfn.Length - i, origfn.Length - (origfn.Length - i));
                        if ((str.Substring(str.Length - 4, 4) == ".nds") || (str.Substring(str.Length - 4, 4) == ".3ds"))
                        {
                            ...
                            if (str.Substring(str.Length - 4, 4) == ".nds")
                            {
                                str3 = str3 + ".nds";
                            }
                            else
        ...
        Label_0116:
            ...
            {
                str = this.fixFileExistsName(str, num2);
                num2++;
            }
            return str;
        }
        ...
        public bool parseCmpGameList()
        {
            ...
            {
                string sKey = this.run.hexAndMathFunction.convertHexToEncryptionKey("3F006F003F003F001000000069003F00");
                FileStream fs = new FileStream("data/databases/cmpGameList.bin", FileMode.Open, FileAccess.Read);
                using (StreamReader reader = new StreamReader(this.encryptor.createDecryptionReadStream(sKey, fs)))
                {
                    string str2;
                    bool flag = false;
                    while ((str2 = reader.ReadLine()) != null)
                    {
                        if (flag)
                        {
                            ...
                            try
                            {
                                ...
                                strArray2 = item.name.Split(new char[] { '(' });
                                item.region = strArray2[1];
        ...
        private unsafe long patchRomBytes(long offset, long endPos, BinaryReader br, BinaryWriter writer, ProgressBar progress, Label status, string romOut)
        {
            ...
            for (int i = 0; i < this.patchInfo.patchlines; i++)
            {
                num2 = int.Parse(this.patchInfo.patchline[i].offset, NumberStyles.HexNumber);
                if (num2 < offset)
                {
                    return -10L;
                }
                ...
                Application.DoEvents();
                if (str != this.patchInfo.patchline[i].find)
                {
                    if (str == this.patchInfo.patchline[i].replace)
                    {
                        return -13L;
                    }
                    return -12L;
                }
                if (this.patchInfo.patchline[i].find.Length != this.patchInfo.patchline[i].replace.Length)
                {
                    return -11L;
                }
        ...
        private long saveRom(string romIn, string romOut, bool apPatch, ProgressBar progress, Label status, long endPos)
        {
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(input))
                {
                    ...
                    if (apPatch)
                    {
                        offset = this.patchRomBytes(offset, endPos, reader, writer, progress, status, romOut);
                        if (offset < 0L)
                        {
                            input.Close();
                            output.Close();
                        ...
                        this.copyBytes(reader, writer, endPos - offset, 0x400, offset, endPos, progress, status, romOut);
                    }
                    else
                    {
                        int num3;
        ...
        public void setupOptionsDisplay()
        {
            if (this.options.getValue("ap_update_check") == "0")
            {
                this.enableUpdateAlertsToolStripMenuItem.Checked = false;
            }
            else
            {
                this.enableUpdateAlertsToolStripMenuItem.Checked = true;
            }
            if (this.options.getValue("cmp_usrcheat_update_check") == "0")
            {
                this.uSRcheatToolStripMenuItem1.Checked = false;
            }
            else
            {
                this.uSRcheatToolStripMenuItem1.Checked = true;
            }
            if (this.options.getValue("cmp_edgecheat_update_check") == "1")
            {
                this.eDGEToolStripMenuItem.Checked = true;
            }
            else
            {
                this.eDGEToolStripMenuItem.Checked = false;
            }
            if (this.options.getValue("cmp_cyclocheat_update_check") == "1")
            {
                this.cycloDSToolStripMenuItem.Checked = true;
            }
            else
            {
                this.cycloDSToolStripMenuItem.Checked = false;
            }
            if (this.options.getValue("disable_update") == "1")
            {
                this.disableCheckOnStartToolStripMenuItem.Checked = true;
            }
            else
            {
                this.disableCheckOnStartToolStripMenuItem.Checked = false;
            }
            if (this.options.getValue("auto_open_collections") != "0")
            {
                this.autoOpenBrowserToolStripMenuItem.Checked = true;
            }
            else
            {
                this.autoOpenBrowserToolStripMenuItem.Checked = false;
            }
            if (this.options.getValue("disable_crc_db") == "1")
            {
            ...
            else
            {
                ...
                if (this.tabControlMain.TabPages[0] != this.tabWebInfo)
                {
                ...
                {
                    this.disableWebAlertContentToolStripMenuItem.Checked = false;
                }
