namespace dsRomHeaderFunctions
{
    ...
    public class dsromHeader
    {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        ...
                        {
                            int num5 = j + (i * 4);
                            int index = this.romHeader.icon.tiles[((num5 * 0x40) + (m * 8)) + k];
                            ...
                            {
                                red = (num7 % 0x20) * 8;
                                green = ((num7 / 0x20) % 0x20) * 8;
                                blue = ((num7 / 0x400) % 0x20) * 8;
                                this.romHeader.icon.gfx.SetPixel((j * 8) + k, (i * 8) + m, Color.FromArgb(0, red, green, blue));
                            }
                            else
                            {
                                red = (num7 % 0x20) * 8;
                                green = ((num7 / 0x20) % 0x20) * 8;
                                blue = ((num7 / 0x400) % 0x20) * 8;
                                this.romHeader.icon.gfx.SetPixel((j * 8) + k, (i * 8) + m, Color.FromArgb(red, green, blue));
                            }
                        }
                    }
                }
        ...
        public void deleteHeaderCacheFile(string hash)
        {
            if (hash != "")
            {
                string path = "data/header_cache/" + hash.Substring(0, 1) + "/" + hash + ".dsrhd";
                bool flag = false;
            ...
            try
            {
                using (sr = new StreamReader(stream))
                {
                    ...
                    if (!flag)
                    {
                        ...
                        str4 = this.readOneLine(sr);
                        string[] strArray = str4.Substring("icon_pal=".Length, str4.Length - "icon_pal=".Length).Split(new char[] { ',' });
                        index = 0;
                        ...
                        str4 = this.readOneLine(sr);
                        string[] strArray2 = str4.Substring("icon_tiles=".Length, str4.Length - "icon_tiles=".Length).Split(new char[] { ',' });
                        index = 0;
                        foreach (string str6 in strArray2)
                        {
                            if (str6 != "")
                            {
                                this.romHeader.icon.tiles[index] = int.Parse(str6);
                                index++;
                            }
                        }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        while (pos < 0x68L)
                                        {
                                            if (!this.skipBytes(reader, &pos, (int) (0x68L - pos)))
                                            {
                                        ...
                                        str3 = this.readHex(reader, &pos, 4);
                                        if (str3 != null)
                                        {
                                            goto Label_02CB;
                                        }
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (this.romHeader.twl_offset == 0)
                                            {
                                                this.romHeader.isDsi = false;
                                            }
                                            else
                                            {
                                                this.romHeader.isDsi = true;
                                            }
                                            hex = this.readHex(reader, &pos, 4);
                                            if (hex == null)
                                            {
                        ...
                        Label_05D3:
                            switch ((pos - num5))
                            {
                                case 0x20L:
                                {
                                    ...
                                    {
                                        this.romHeader.icon.tiles[n] = int.Parse(hex.Substring(n, 1), NumberStyles.HexNumber);
                                    }
                                    goto Label_081C;
                                }
                                case 0x220L:
                                    for (int num9 = 0; num9 < 0x10; num9++)
                                    {
                                        ...
                                        this.romHeader.icon.pal[num9] = hex;
                                    }
                                    goto Label_081C;
                            ...
                            {
                                this.romHeader.icon.tiles[k * 2] = int.Parse(hex.Substring((k * 2) + 1, 1), NumberStyles.HexNumber);
                                this.romHeader.icon.tiles[(k * 2) + 1] = int.Parse(hex.Substring(k * 2, 1), NumberStyles.HexNumber);
                            }
                        Label_081C:
                            if ((pos - num5) < 0x840L)
                            {
                            ...
                            {
                                for (int num13 = 0; num13 < 0x88; num13++)
                                {
                                    hex = this.readHex(reader, &pos, 1);
                                    if ((hex != "FF") && (hex != "00"))
                                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (this.readHex(reader, &pos, 8) != null)
                                {
                                    goto Label_043E;
                                }
                                this.failedLoad("13", batch);
        ...
        public void saveHeaderCacheFile()
        {
            if (this.romHeader.hash != "")
            {
                ...
                writer.Write("\r\n");
                writer.Close();
                string str5 = Program.form.run.hexAndMathFunction.convertHexToEncryptionKey("790077003F0028003F0050003F003F00");
                GCHandle gch = GCHandle.Alloc(str5, GCHandleType.Pinned);
                Program.form.encryptor.EncryptFile(str + ".txt", str + ".dsrhd", str5, gch);
